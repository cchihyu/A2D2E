...
def _estimate_beta(f_hat, V, V_tilde, d, delta, D):
    """
    Closed-form OLS slope for variable d:
...
def dale_curve(f_hat, X_train, d, n_grid=100, K=40, grad_fn=None, eps=1e-4):
    N, D = X_train.shape
    data_d = X_train[:, d]
    boundaries = _bin_boundaries(data_d, K)
    K_actual = len(boundaries) - 1
    ...
    if grad_fn is not None:
        ...
        grads = (f_hat(X_hi) - f_hat(X_lo)) / (2.0 * eps)
